...
<!-- ## 🦠 **Understanding the SIR Case Study**

This case study explores how the **final epidemic size** changes as we vary the two key parameters in the stochastic SIR model:

- **α (alpha)** — infection probability per contact  
- **β (beta)** — removal/recovery probability per infected individual  

The simulation uses a **discrete‑time stochastic SIR model**:

- At each time step:
  - Each susceptible avoids infection with probability $(1 - \alpha)^I$, where $I$ is the number of infectious individuals.
  - Each infected individual recovers with probability $\beta$.
- The process runs for **T = 100** time steps in a population of **N = 1000**.

The goal is to estimate:

$$
E[S(T)] \quad \text{(expected number of susceptibles at time T)}
$$

This quantity is directly related to the **epidemic size**:

$$
\text{Epidemic size} = N - E[S(T)]
$$

A **smaller** $E[S(T)]$ means **more people were infected** over the course of the epidemic.


## 🌐 **Explaining the Final 3D Plot: $E[S(T)]$ over α and β**

The last plot is a **wireframe surface** showing how the expected number of susceptibles at time $T$ varies across a grid of α and β values.

### Axes

- **x‑axis: α** (infection probability)  
- **y‑axis: β** (recovery probability)  
- **z‑axis: $E[S(T)]$** (expected susceptibles at time T)

### What the surface shows

- **Increasing α** (moving right) → more infections → **lower $E[S(T)]$**  
- **Increasing β** (moving forward) → faster recovery → **higher $E[S(T)]$**  

### Interpretation of the shape

- The surface slopes **downward** as α increases:  
  More infectiousness → larger epidemic → fewer susceptibles left.

- The surface slopes **upward** as β increases:  
  Faster recovery → smaller epidemic → more susceptibles remain.

- The combined effect creates a **smooth, curved surface** where:
  - **Low α + high β** → epidemic barely spreads → $E[S(T)]$ close to 1000  
  - **High α + low β** → epidemic spreads widely → $E[S(T)]$ much smaller  

### What the colour gradient represents

- Light colours (yellow) = **large $E[S(T)]$** → small epidemic  
- Dark colours (blue) = **small $E[S(T)]$** → large epidemic  

This visualises the **epidemic size landscape** across the parameter space. -->
